...
import workers.cleanup_worker as cleanup_worker_module
...
def test_cleanup_deletes_old_files(tmp_path, monkeypatch):
    monkeypatch.setattr(cleanup_worker_module, "AUDIO_DIR", str(tmp_path))
    ...
    result = cleanup_worker_module.cleanup_audio.run()
...
def test_cleanup_no_directory(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cleanup_worker_module, "AUDIO_DIR", missing_dir)
