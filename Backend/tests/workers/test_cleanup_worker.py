...
def test_cleanup_deletes_old_files(tmp_path, monkeypatch):
    monkeypatch.setattr("workers.cleanup_worker.settings.AUDIO_DIR", str(tmp_path))
    monkeypatch.setattr("workers.cleanup_worker.settings.AUDIO_FILE_TTL_SECONDS", 3600)
...
def test_cleanup_no_directory(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("workers.cleanup_worker.settings.AUDIO_DIR", missing_dir)
