...
import workers.narrative_worker as narrative_worker_module


def test_narrate_saves_mp3_and_returns_url(tmp_path, monkeypatch):
    monkeypatch.setattr(narrative_worker_module, "AUDIO_DIR", str(tmp_path))

    mock_provider = MagicMock()
    mock_provider.synthesize = AsyncMock(return_value=b"fake_mp3_bytes")
    monkeypatch.setattr(narrative_worker_module.TTSFactory, "get_provider", lambda: mock_provider)

    task = narrative_worker_module.narrate
